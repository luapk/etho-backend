...
**YOLO as measurement oracle:** `yolo_pose_service.py` samples video at 5 fps, runs `yolo11m.pt` (detection, classes 15=cat 16=dog; `YOLO_MODEL` env-configurable) and `yolo11n-pose.pt` (skeleton) separately, then matches skeletons to pet bounding boxes by IoU. The derived metrics — spinal curvature (degrees deviation from nose→shoulder→hip axis), head tilt, detection coverage — are injected as a `## YOLO11-POSE MEASUREMENTS` block in the Pass 2 prompt. Gemini is instructed to cite these measurements when making posture claims rather than using vague language. YOLO uses human-pose keypoints (COCO-17) applied to animals, which can produce **confident nonsense** — measured on a clip of a calm sitting pug: mean spinal curvature 75°, SD 51°, head tilt −175° (upside down), interpreted as "extreme fear crouch". Pose-derived angles therefore pass **reliability gates** before being reported: stability (SD ≤ 20°, since real posture doesn't swing tens of degrees per second) and plausibility (mean ≤ 45°, since the interpretation scale ends at 30°; head tilt |mean| ≤ 90°). Failing either, raw values are kept for debugging but marked `reliable: false`, no interpretation is produced, the indexed DB columns are left null, and Pass 2 is told the measurement is unavailable and to assess visually. The real fix is an animal-trained pose model (AP-10K / SuperAnimal) — `scripts/compare_pose_models.py` runs that comparison.
