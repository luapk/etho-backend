...
Quickly analyze this pet video. Identify:
1. Species and breed (apply morphological normalization)
2. Overall emotional state (distress score 0-100)
3. Key behaviors observed with FACS codes
4. Any vocalizations

Return brief JSON with overall_assessment, key observations, and morphology notes.
"""

AUDIO_ANALYSIS_PROMPT = """
Focus on audio analysis of this pet video using:
- Morton's Motivation-Structural Rules (1977)
- Canine Bio-Acoustics (Pongrácz, Faragó et al.) for dogs
- Meowsic Project (Schötz et al.) for cats

For each vocalization, identify:
1. Timestamp
2. Type (bark, meow, growl, whine, purr, etc.)
3. Pitch characteristics (low/medium/high, tonal/noisy)
4. Probable meaning based on research
5. Breed-specific notes (e.g., Siamese high vocalization is normal)

Return JSON with vocalizations_detected array.
"""
